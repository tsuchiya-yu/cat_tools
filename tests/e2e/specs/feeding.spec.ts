...
test.describe('猫の給餌量計算 E2E', () => {
  ...
  test('複数フードの分量が0以下だとエラーになる', async ({ page }) => {
    await page.goto(PAGE);
    await page.fill('#kcalInput', '200');
    await page.fill('#densityInput', '400');
    await page.getByRole('button', { name: '＋ フードを追加' }).click();
    ...
    const ratioInputs = page.getByLabel('与えたい配分（g）');
    await ratioInputs.nth(0).fill('0');
    await expect(page.getByText('0より大きい値を入力してください。').first()).toBeVisible();
  });
  ...
  test('複数フード: 追加・最大5件・6件目不可・削除で単一UIへ戻る', async ({ page }) => {
    ...
    await expect(page.getByLabel('フード名（任意）').nth(0)).toHaveValue('');
    await expect(page.getByLabel('フード名（任意）').nth(1)).toHaveValue('');
