...
export const CAT_BCS_CHECK_UI_TEXT = {
  ...
  PRECHECK: {
    TITLE: 'チェックする前に',
    ITEMS: [
      '家庭での観察目安であり、獣医師の診断ではありません',
      '長毛猫は毛を分けて触診してください',
      '妊娠中・急な体重変化・食欲低下などがある場合は受診を優先してください',
      '子猫の給餌量はBCSだけで自己調整しないでください',
      'BCSは脂肪の目安で、筋肉量（MCS）は評価しません',
    ],
  },
  PALPATION_GUIDE: {
    TITLE: '触診のしかた',
...
export const FEEDING_UI_TEXT = {
  HEADER: {
    ...
    TITLE: '猫の給餌量計算',
    DESCRIPTION:
      '1日の必要カロリーとフードのカロリー密度（kcal/100g）から、1日に与える量（g）を自動計算します。2〜5種類のフードを混ぜる場合も、与えたいグラム数の比率を指定してそれぞれの目安量を確認できます。結果はあくまで目安なので、便の状態や体重推移を見ながら調整してください。',
  },
  GUIDE: {
    WHAT_TITLE: 'このツールでできること',
    WHAT_DESCRIPTION:
      '1日の必要カロリーと、フード100gあたりのカロリー（kcal/100g）から、1日に与える量（g）を自動計算できます。朝・夜の配分目安も同時に確認でき、最大5種類までの複数フードにも対応しています。',
    USAGE_TITLE: '使用例（こんなときに使えます）',
    USAGE_ITEMS: [
      '今のフードを1日何g与えるべきか、すぐに計算したいとき',
      '朝夕2回に分ける際の1回あたりの量を決めたいとき',
      '2種類以上のフードを混ぜるとき、普段の配分を保ったまま各フードの1日量を知りたいとき',
  ...
  RESULT: {
    TITLE: '1日に与える目安量',
    MULTI_TITLE: '1日の給餌量の目安',
    NOTE: '※あくまで目安です。猫の体型や活動量に合わせて少しずつ調整してください。',
    MULTI_NOTE: '実際に量る際は、使用しているスケールの精度に合わせて調整してください。',
    MULTI_SAFETY:
      'この計算は、入力したフードのカロリーと重量から給餌量の目安を算出するものです。複数のフードを組み合わせた際の栄養バランスそのものを判定するものではありません。主食として与える場合は、各製品の表示や対象ライフステージを確認してください。療法食を使用している場合は、自己判断で他のフードと混ぜず、獣医師の指示を優先してください。',
  },
  MULTI_FOOD: {
    ADD_BUTTON: '＋ フードを追加',
    ...
    NIGHT_AMOUNT: (grams: string) => `夜: 約${grams}g`,
  },
  LINKS: {
...
export const FEEDING_FAQ_ITEMS = [
  ...
  {
    question: 'ドライフードとウェットフードを一緒に計算できますか？',
    answer:
      'はい。どちらもkcal/100gが分かれば、計算上は同じように利用できます。なお、「1袋あたり○kcal」しか分からない商品を自動で100g換算する機能は、このツールの対象外です。',
  },
] as const;
